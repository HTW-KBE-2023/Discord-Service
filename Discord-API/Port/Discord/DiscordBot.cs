...
namespace API.Port.Discord
{
    public class DiscordBot : IDiscordBot
    {
        ...
        private async Task Client_Ready()
        {
            ...
            foreach (var item in _commandRegister.SlashCommands)
            {
                _logger.LogInformation("Registered SlashCommands -> {name}", item.Name);
            }
        }
        ...
        private static (EmbedBuilder embedBuiler, ComponentBuilder componentBuilder) SurveyToDiscordMessageComponents(Survey survey)
        {
        ...
        public async Task ReplyToUserForConcludedFight(Fight fight, DiscordUser user)
        {
            var discordUser = _client.GetGuild(1104714955468574805).GetTextChannel(1156253208574955610).GetUser(user.DiscordId);
